...
public void checkMove(int playerIdx, int[] cardIdx){
    if (cardIdx != null){//check if the input is valid or not
        ...
        if (tempHand != null){//check if it is valid hand or not: only when it is in the three situation below can one play their hands, not a legal hand otherwise
            if (handsOnTable.isEmpty() && !tempHand.contains(new Card(0, 2))){
                ui.printMsg("not a legal move!!!\n");
            ...
            if (handsOnTable.isEmpty() || tempHand.beats(handsOnTable.get(handsOnTable.size()-1)) || handsOnTable.get(handsOnTable.size() - 1).getPlayer() == playerList.get(playerIdx) ){
                ...
                ui.printMsg("{"+tempHand.getType()+"} "+tempHand.toString()+"\n");
        ...
        String result;
            ...
            for (int i = 0; i<numOfPlayers;i++){
                if (playerList.get(i).getNumOfCards()!=0){
                    result+=playerList.get(i).getName()+" has "+String.valueOf(playerList.get(i).getNumOfCards())+" cards in hand.\n";
                }
                else{
                    result+=playerList.get(i).getName()+ " wins the game.\n";
                }
            }
        JOptionPane.showMessageDialog(ui.getbigTwoPanel(),result);
    }
    ui.repaint();
